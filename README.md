...
Бот использует только стандартную библиотеку Python, поэтому образ получается маленьким (~79 МБ) и потребляет около 16 МБ RAM.
...
`config.json` монтируется только для чтения, каталог `logs/` — на запись, поэтому логи остаются на сервере после пересоздания контейнера. После изменения `config.json` достаточно `docker compose restart`.

Ограничения ресурсов заданы в `docker-compose.yml`: 0.5 CPU и 128 МБ RAM. Контейнер запускается от непривилегированного пользователя и поднимается автоматически после перезагрузки сервера (`restart: unless-stopped`).
